/*
 * Copyright (c) Sakion Team. All rights reserved.
 *
 * File name: rekernel_main.c
 * Description: Re:Kernel init/exit. Brings up the netlink transport, then
 *              registers the binder / signal / netfilter / kprobe hooks.
 * Author: nep_timeline@outlook.com
 * Last Modification:  2026/06/28
 */
#include "rekernel_internal.h"

static int __init start_rekernel(void)
{
	int rc;

	pr_info("Thank you for choosing Re:Kernel!\n");
#ifdef DEBUG
	pr_info("Debug mode is enabled!\n");
#endif

	rc = rekernel_netlink_start();
	if (rc != LINE_SUCCESS)
		return rc;

	pr_info("Re-Kernel start hooking!\n");

	rc = register_binder();
	if (rc != LINE_SUCCESS) {
		pr_err("%s: Failed to hook binder!\n", __func__);
		goto err_netlink;
	}

	rc = register_signal();
	if (rc != LINE_SUCCESS) {
		pr_err("%s: Failed to hook signal!\n", __func__);
		goto err_binder;
	}

	if (IS_ENABLED(CONFIG_REKERNEL_NETWORK)) {
		rc = register_netfilter();
		if (rc != LINE_SUCCESS) {
			pr_err("%s: Failed to hook netfilter!\n", __func__);
			goto err_signal;
		}
	}

#ifdef CLEAN_UP_ASYNC_BINDER
	rc = register_kp();
	if (rc != LINE_SUCCESS) {
		pr_err("%s: Failed to hook kprobe!\n", __func__);
		goto err_netfilter;
	}
#endif

	pr_info("Re-Kernel hooked!\n");
	return LINE_SUCCESS;

err_netfilter:
	if (IS_ENABLED(CONFIG_REKERNEL_NETWORK))
		unregister_netfilter();
err_signal:
	unregister_signal();
err_binder:
	unregister_binder();
err_netlink:
	rekernel_netlink_stop();
	return rc;
}

static void __exit exit_rekernel(void)
{
	pr_info("Re-Kernel closing...\n");
	unregister_kp();
	if (IS_ENABLED(CONFIG_REKERNEL_NETWORK))
		unregister_netfilter();
	unregister_signal();
	unregister_binder();
	rekernel_netlink_stop();
}

device_initcall(start_rekernel);
module_exit(exit_rekernel);

MODULE_LICENSE("GPL");
